...
function dataTrainValid = DBNModel_TVT(inter_structure, dataTrain, dataTrainMiss, dataValid, dataTest, ns, dag, max_iter, intraLength, horizon, numNodes,ncases) %add dataTest to this function
      ...
      disp('Starting Inference, Which May Take Some Time...');
          ...
          casesTest = data2cell(dataTest, ss, T,to_replace=-1);
          dataTestMarginals = dbnInference(casesTest, ss, T, ncases=size(dataTest)(1), bnet=bnet2, numNodes);
          %dataTestMarginals = dbnInference(casesTest, ss, T, ncases, bnet=bnet2, numNodes); %uncomment to run inference on subset of ncases


          dataTrainValid = [dataTrainMarginals; dataValidMarginals; dataTestMarginals];
          %dataTrain = dataTrainMarginals;
          %dataValid = dataValidMarginals;
          %dataTest = dataTestMarginals;
end
