...
def DBN_NodeSize(df,intra_cols,inter_cols):
    ...
    for l in t4:
        ...
        ns_list4.append(ns)
        count+=1
    ns_list5 = [max((w,x,y,z),key=abs) for w,x,y,z in zip(ns_list1,ns_list2,ns_list3,ns_list4)]

    return ns_list5

def dbn_cols_inter(intra_cols,sequence_length_dbn,cols_start,new_cols_start,cols_end,newer_cols_start,outcome_name):
    """
    ...
    for i in range(len(inter_cols)):
        if outcome_name == 'egfr_reduction40_ge':
            if inter_cols[i].startswith(cols_start):
                ...
                targets = ["year" + str(m) + "_" + "reduction_40_ge" for m in range(2,sequence_length_dbn+2)]
        elif outcome_name == 'AKI_BOS24':
            if inter_cols[i].endswith(cols_start):
                inter_cols[i] = inter_cols[i].replace(cols_start, new_cols_start)
                liss.append(inter_cols[i])
            else:
                targets = ["aki_progression_" + str(m) + "days" for m in range(2,sequence_length_dbn+2)]
        elif  outcome_name == 'AKI_BOS48':
            if inter_cols[i].endswith(cols_start):
                inter_cols[i] = inter_cols[i].replace(cols_start, new_cols_start)
                liss.append(inter_cols[i])
            else:
                targets = ["aki_progression_" + str(m) + "days" for m in range(3,sequence_length_dbn+3)]
        elif outcome_name == 'AKI_BOS72':
            if inter_cols[i].endswith(cols_start):
                inter_cols[i] = inter_cols[i].replace(cols_start, new_cols_start)
                liss.append(inter_cols[i])
            else:
                targets = ["aki_progression_" + str(m) + "days" for m in range(4,sequence_length_dbn+4)]

    for j in range(0,len(liss)):
        if outcome_name == 'egfr_reduction40_ge':
            if liss[j].startswith(newer_cols_start):
                K = len(intra_cols) - 1
                inter_colss = ["year" + str(m) for m in range(1,sequence_length_dbn+1)] #add 1 to include final timepoint in cols
            ...
            if liss[j].endswith(newer_cols_start):
                K = len(intra_cols) - 1
                inter_colss = ["24hourperiod_" + str(m) for m in range(0,sequence_length_dbn+1)] #add 1 to include final timepoint in cols
                res =  [ele for ele in inter_colss for y in range(K)]
...
def intra_struct(df,intra_cols,ns_list,method,outcome_name,site,adjusted,track,max_fan_in):
    """
...
def BNModel_TT(df,df2,intra_cols,max_iter,dag,ns_list,method,outcome_name,site,adjusted,sequence_length_bn,track,ncases):
    """
    ...
    dataTrainValid = octave.BNModelLearn_TT(dag, intraLength, numNodes, max_iter, dataTrain, dataValid, ns_list,horizon,ncases)
    path = './BNModel'

    np.save(path + '/' +  method + '_BNModel_' + outcome_name + '_' + site + adjusted+ '_' + track, dataTrainValid)

    #print(dag)

    return dataTrainValid

def BNModel_TVT(df,df2,df3,intra_cols,max_iter,dag,ns_list,method,outcome_name,site,adjusted,sequence_length_bn,track,ncases):
    """
    function that learns the BN model, parameterizes the network, and produces the marginal probabilities
    ...
    dataTest = df3.values
    print("Starting BNT Function")
    dataTrainValid = octave.BNModelLearn_TVT(dag, intraLength, numNodes, max_iter, dataTrain, dataValid, dataTest, ns_list,horizon,ncases)
    path = './BNModel'
...
def inter_struct(df,inter_cols,ns_list,method,outcome_name,site,adjusted,sequence_length_dbn,track,max_fan_in):
    """
...
def DBNModel_TT(df,df2,inter_cols,inter_structure,sequence_length_dbn,max_iter,dag,ns_list,method,outcome_name,site,adjusted,track,ncases):
    """
    ...
    print("Starting BNT Function")
    dataTrainValid = octave.DBNModel_TT(inter_structure, dataTrain, dataTrainMiss, dataValid, ns_list, dag, max_iter, intraLength, horizon, numNodes,ncases)
    path = './DBNModel'

    np.save(path + '/' +  method + '_DBNModel_' + outcome_name + '_' + site + adjusted + '_' + track, dataTrainValid)

    #print(dag)

    return dataTrainValid

def DBNModel_TVT(df,df2,df3,inter_cols,inter_structure,sequence_length_dbn,max_iter,dag,ns_list,method,outcome_name,site,adjusted,track,ncases):
    """
    function that learns the DBN model, parameterizes the network, and produces the marginal probabilities
    Note: use this function when you have train/valid/test (TVT) split
    ...
    print("Starting BNT Function")
    dataTrainValid = octave.DBNModel_TVT(inter_structure, dataTrain, dataTrainMiss, dataValid, dataTest, ns_list, dag, max_iter, intraLength, horizon, numNodes,ncases)
    path = './DBNModel'
...
def draw_intra_structure(adjacency_matrix,mylabels,method,outcome_name,site,adjusted,track,clipback,clipfront):
    """
    ...
    figure1 = plt.savefig(path + '/' +  method + '_intra_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.svg', dpi=1200, format='svg')
    figure2 = plt.savefig(path + '/' +  method + '_intra_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.png', dpi=1200, format='png')

    return figure1, figure2

def draw_inter_structure(adjacency_matrix_2,mylabels,method,outcome_name,site,adjusted,track,clipback,clipfront):
    """
    function that generates and saves the learned structures in graph visualization form
    ...
    figure1 = plt.savefig(path + '/' +  method + '_inter_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.svg', dpi=1200, format='svg')
    figure2 = plt.savefig(path + '/' +  method + '_inter_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.png', dpi=1200, format='png')

    return figure1,figure2


def draw_compact_structure(adjacency_matrix_1,adjacency_matrix_2,mylabels,method,outcome_name,site,adjusted,track,clipback,clipfront):
    """
    function that generates and saves the learned structures in graph visualization form
    ...
    figure1 = plt.savefig(path + '/' +  method + '_compact_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.svg', dpi=1200, format='svg')
    figure2 = plt.savefig(path + '/' +  method + '_compact_structure_visualization_' +outcome_name + '_' + site + adjusted+'_' + track + '.png', dpi=1200, format='png')
